        ...
        return (
            <Featured>
                <div className="container">
                    <div className="row">
                        <h5 className="font-weight-bold text-uppercase title-featured">Featured Products</h5>
                    </div>
                    <div className="row">
                        <ProductConsumer>
                            {(value) => {
                                const { featuredProducts: products, handleDetail, addToCart, addToCompare, openModal, openListModal, compare } = value;
                                return (
                                    <SliderWrapper className="col-12 mx-auto">
                                        <Slider {...settings}> {(products.map(product => {
                                            return (
                                                <ProductWrapper key={product.id} className="">
                                                    <div className="card">
                                                        {product.newest === true ? <div className="product-label">new</div> : ""}
                                                        {product.offer !== 0 ? <div className="offer">{product.offer}% off</div> : ""}
                                                        <Link to="/details">
                                                            <div className="img-container p-2 mt-1" onClick={() => { handleDetail(product.id) }}>
